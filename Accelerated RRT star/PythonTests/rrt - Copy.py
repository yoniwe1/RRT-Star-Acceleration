...
class RRT():
    ...
    def Planning(self, animation=True):
        ...
        for i in range(self.maxIter):
            ...
            newNode = self.steer(rnd, nind)#connect new to nearest in the right direction
            #  print(newNode.cost)

            if self.__CollisionCheck(newNode, self.obstacleList):#false means there is collision
    ...
    def steer(self, rnd, nind):
        ...
        theta = math.atan2(rnd[1] - nearestNode.y, rnd[0] - nearestNode.x)# direction to random XY
    ...
    def get_best_last_index(self):# return index of the cheapest point that enough close to the destination
        ...
        disglist = [self.calc_dist_to_goal(
            node.x, node.y) for node in self.nodeList]# array of disance from all points to destination
        goalinds = [disglist.index(i) for i in disglist if i <= self.expandDis]# i= distance
        # goalinds contains the indexes of all good distances in disglis( smaller than step size)
    ...
    def find_near_nodes(self, newNode):# return all points in tree that are neighbors of our new Node
        ...
        r = 50.0 * math.sqrt((math.log(nnode) / nnode))#(smaller than 1)    *  ( 50)
    ...
    def check_collision_extend(self, nearNode, theta, d):
        ...
        for i in range(int(d / self.expandDis)):#check path for collision
            tmpNode.x += self.expandDis * math.cos(theta)
            tmpNode.y += self.expandDis * math.sin(theta)
            if not self.__CollisionCheck(tmpNode, self.obstacleList):
                return False# there is obstacle
    ...
    def __CollisionCheck(self, node, obstacleList):#newnode
